	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			{isEditing ? (
				...
				<Button className="w-fit self-center" onClick={() => setOpen(true)}>
					Create Goal
				</Button>
			)}
			<DialogContent>
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
						<DialogHeader>
							...
							<DialogDescription>
								{isEditing
									? 'Make changes to your goal here. Click "Save Changes" when you’re done.'
									: 'Create a new goal. Click save when you’re done.'}
							</DialogDescription>
						</DialogHeader>
						<FormField
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem className="flex items-center gap-2 py-4">
											<FormControl>
												<Button type="button" variant={field.value ? 'default' : 'outline'} onClick={() => field.onChange(!field.value)}>
